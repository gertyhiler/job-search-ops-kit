...
- Escalation rules from the runtime config root (`~/.config/job-search/escalation-rules.yaml` by default).
...
3. Submit via MCP `propose_strategy_change`. Then call MCP `auto_decide_strategy`. Respect whatever the deterministic evaluator returns:
...
6. Write the human-readable before/after summary via MCP `write_journal_entry`.

## Output

- One proposal, one decision, one optional apply call.
- Human-readable summary via MCP `write_journal_entry` with before/after diff and confidence.
